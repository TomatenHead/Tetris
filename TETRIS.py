...
class Figure:
    ...
    Figures = [
       ...
       [[1,2,5,9],[0,4,5,6],[1,5,9,8],[4,5,6,10]], # J Blochu
        ...
        for i in range(4):
            for j in range(4):
                ...
                if p in self.Figure.image():
                    if j + self.Figure.x + dx -1 > self.width or \
                       j + self.Figure.y + dx < 0:
                        edge = True
        if not edge:
            self.Figure.x += dx
        if self.intersects():
...
while not done:
    ...
    text_score = gameover_font.render("Score" + str(game.score),True,(255,215,0))
